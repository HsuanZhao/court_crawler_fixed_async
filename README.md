...
项目简介
本工具用于抓取上海市高级人民法院网站（https://www.hshfy.sh.cn）的公开裁判文书数据。包含两个主要脚本：
...
主抓取程序 (sh_court_fixed_async.py)自动访问上海市高级人民法院文书查询页面，模拟表单提交获取文书列表，提取文书基本信息（案号、标题、文书类型、案由、审判部门、审级、结案日期等），打开详情页获取完整文书内容，并支持JSON和CSV格式数据导出

诊断工具 (debug_page_structure.py)分析网站页面实际HTML结构，识别所有表格元素和数据行，检测可能的文书链接，生成详细的诊断报告和截图。

网站反爬虫技术及破解方法：

1. 动态表单提交
反爬技术：
搜索表单通过JavaScript动态处理，无法直接通过URL访问结果页，提交后需要等待JavaScript渲染才能看到数据

破解方法：
python
# 使用Playwright模拟真实浏览器行为
await page.evaluate("document.querySelector('form').submit()")
await page.wait_for_timeout(5000)  # 等待JavaScript渲染

2. 加密URL参数
反爬技术：
详情页URL参数采用加密处理，无法直接构造，点击事件通过onclick属性触发JavaScript函数
...
3. 内容动态加载
反爬技术：
文书列表数据通过AJAX动态加载，传统requests库无法获取完整数据

破解方法：
python
# 使用Playwright等待网络空闲状态
...
技术亮点
异步架构优化
使用asyncio和playwright实现异步操作，提高效率
...
# 4. 运行主抓取程序
python sh_court_fixed_async.py
配置说明
主程序配置参数：
...
免责声明
本工具仅用于技术研究和学习目的。使用者应遵守相关法律法规和网站使用条款，对使用本工具产生的任何后果自行承担责任。开发者不对因使用本工具造成的任何直接或间接损失负责。
